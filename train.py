...
def train(args):
    ...
    for i_episode in  range(1, 1+args.num_episodes):
        ...
        print("\rEpisodic {} Score: {:.4f}\t Avg Score: {:.4f}".format(i_episode, episode_score,latest_avg_score),end=' ')
        
        if max_score <= episode_score:                     
            max_score = episode_score
            agent.save(args.model_path)                     # save best model so far
        
        if len(total_rewards) >= args.test_n_run:                       # record avg score for the latest 100 steps
            latest_avg_score = sum(total_rewards[(len(total_rewards)-args.test_n_run):]) / args.test_n_run
            ...
            if max_avg_score <= latest_avg_score:           # record better results
                ...
                if max_avg_score > 0.5:                     
                    ...
                    agent.load(args.model_path)             # continue from last best-model
