...
from models_soccer import ActorNetwork, CriticNetwork   
...
class MADDPG:
    ...
    def __init__(self,
        ...
        agent1 = DDPGAgent(*args, **kargs)
        agent2 = DDPGAgent(*args, **kargs)
        self.adversarial_agents = [agent1, agent2]     # the agent self-plays with itself
    ...
    def save(self, id, path):
        ...
        Save the model
        """
        agent1 = self.adversarial_agents[id]
        torch.save((agent1.actor_local.state_dict(), agent1.critic_local.state_dict()), path+"_1")

    def load(self, id, path):
        """
        Load model and decay learning rate
        """
        actor_state_dict, critic_state_dict = torch.load(path+"_{}".format(id+1))
        agent = self.adversarial_agents[id]
        ...
        self.adversarial_agents[id] = agent
